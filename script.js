...
function updateCityElements(city) {
    ...
    if (counter) {
        const count = baseCounts[city] || baseCounts['Nigeria'];
...
function detectCity() {
    ...
    fetch('https://ipapi.co/json/')
        ...
        .then(data => {
            ...
            // Попытка 2: ip-api.com если ipapi.co не дал нужный город
            tryIpApi();
        })
        .catch(() => {
            tryIpApi();
...
function generateTickerNotification() {
    const cityNames = names[detectedCity] || names['Nigeria'];
    ...
    const prize = action.includes('won') ? tickerPrizes[Math.floor(Math.random() * tickerPrizes.length)] : '';
    const timeAgo = action === 'playing now' ? 'just now' : `${Math.floor(Math.random() * 10) + 1} min ago`;
